...
router = APIRouter(prefix="/api/worker/earnings", tags=["worker-earnings"])


@router.get("/")
async def earnings(request: Request, identity: dict = Depends(require_worker)):
...
@router.get("/summary")
async def summary(request: Request, identity: dict = Depends(require_worker)):
    wid = await worker_id(request, identity)
    async with require_pool(request).acquire() as conn:
...
@router.get("/recent")
async def recent(request: Request, identity: dict = Depends(require_worker)):
    wid = await worker_id(request, identity)
    async with require_pool(request).acquire() as conn:
...
@router.get("/payouts")
async def payouts(request: Request, identity: dict = Depends(require_worker)):
    wid = await worker_id(request, identity)
    async with require_pool(request).acquire() as conn:
